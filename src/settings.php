...
$API_key = 'MY_ZOTERO_API_KEY';      // obtain from https://www.zotero.org/settings/keys [no default]
$user_ID = 'MY_ZOTERO_USER_ID';      // obtain from https://www.zotero.org/settings/keys [no default]

/**
 * User / Library slug from the Zotero site
 * @global string $user_name
 */
$user_name = 'MY_ZOTERO_USER_NAME';
...
$cache_base_URL = "MY_ZOTERO_CACHE_BASE_URL";           // [default=""]; if the cache directory is NOT a subdirectory of the directory which contains the script AND it (the cache) is web accessible, provide its URL here for use to display web snapshots. if the cache directory is not a subdirectory of the script's own directory and nothing it provided here, display of web snapshots will not work (all other attachment types are unaffected) 
